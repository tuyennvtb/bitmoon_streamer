...
class HakkaFinanceWS extends CoinAbstract {
  constructor(option) {
    super(option);
    const mode = option.mode || 'production';
    // Setup coin server
    if (mode === 'production' || mode === 'uat') {
      this.uri = service.erc20.livenet;
      this.apiUri = service.erc20.liveApi;
    } else {
      this.uri = service.erc20.technet;
  ...
  getSubscribeMsg = async () => {
    ...
    const walletToSubscribe = [];
    wallets.forEach(wallet => walletToSubscribe.push(wallet.coin_address));
    walletToSubscribe.push(this.hotWallet.wallet_address);
    const fromBlock = await this.getLast200Block();
    const message = {
        fromBlock: fromBlock,
        filter: {
          to: walletToSubscribe
         },
    };
    return message;
  };
  ...
  getContractAddress = async() => {
      return "0x0E29e5AbbB5FD88e28b2d355774e73BD47dE3bcd";
  }
  ...
  isTransaction = async() => {
    ...
    try{
      if (
        'transactionHash' in this.data &&
        'event' in this.data &&
        'returnValues' in this.data &&
        'address' in this.data &&
        'blockHash' in this.data &&
        this.data.address.toLowerCase() == contractAddress.toLowerCase()
      ) {
        result = (this.data.event == 'Transfer' && !this.data.removed)
      }
    }catch(error){
      console.log(`[HakkaFinanceWS] - isTransaction() catch transaction error ${error}`);
    }
    return result;
  };
